...
// Meteor.setTimeout(() => {
//   const list = dbPrice.find({
//     createdAt: {
//       $lt: new Date( Date.now() - 600*1000 )
//     }
//   }).fetch();
//   console.log("dbPrice", list);
// }, 5000);

function doSomethingAfterRandomTime(userId) {
  const randomTime = randomNumber(600000, 60000);
  Meteor.setTimeout(() => {
    // try {
    //   doSomething(userId);
    // }
...
function doSomething(userId) {
  ...
  else if (foundationNumber > 0 && probability(1)) {
    const investMoney = randomNumber(Math.round(user.profile.money / 10), 1);
  ...
  let orderList = dbOrders.find({userId}).fetch();
  while (true) {
    if (!orderList) {
      break;
    }
    if (probability(orderList.length * 5)) {
    ...
    orderList = dbOrders.find({userId}).fetch();
  }
  ...
  if (canSellStockList.length > 0) {
    ...
    if (probability(5)) {
      ...
      createSellOrder(user, {
        companyId: companyId,
        unitPrice: randomNumber(companyData.listPrice * 1.15, Math.ceil(companyData.listPrice * 0.85)),
  ...
  const canBuyStockCompanyList = dbCompanies
    ...
    .fetch();
  if (canBuyStockCompanyList.length > 0 && probability(5)) {
    ...
    if (user.profile.money > minPrice) {
      ...
      const maxPrice = Math.min(useMoney, companyData.listPrice * 1.15);
